...
public class VariableReferenceExpression extends Expression {
    ...
    public <T> T accept(MutableExpressionVisitor<T> visitor, ExpressionHandle handle) {
        return visitor.visitVariableReferenceExpression(handle);
    }

    @Override
    public Expression copy(Map<Variable, Expression> substitution) {
        Expression e = substitution.get(variable);
        return e == null ? new VariableReferenceExpression(ctx, variable) : e.copy();
    }
}