...
public class FnOneOrMoreIterator extends AbstractLazilyEvaluatedFunctionIterator {
    ...
    public FnOneOrMoreIterator(RegisterAllocator rAllocator, Function fn, RuntimeIterator[] arguments,
            StaticContext ctx) {
    ...
    public Object next(CallStackFrame frame) throws SystemException {
        XDMItem i = (XDMItem) arguments[0].next(frame);
        if (first.get(frame)) {
            first.set(frame, false);
            if (i == null) {
