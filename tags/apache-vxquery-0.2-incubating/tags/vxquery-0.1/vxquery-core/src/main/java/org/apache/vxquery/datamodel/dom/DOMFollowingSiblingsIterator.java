...
final class DOMFollowingSiblingsIterator implements CloseableIterator {
    ...
    DOMFollowingSiblingsIterator(DOMNode focus) {
        this.focus = focus;
        next = DOMNode.wrap(focus.getDOMManager(), focus.getWrappedObject().getNextSibling(), (DOMNode) focus
                .getParent());
    }
    ...
    public Object next() throws SystemException {
        ...
        next = DOMNode.wrap(focus.getDOMManager(), next.getWrappedObject().getNextSibling(), (DOMNode) focus
                .getParent());
        return oldNext;
    }
}