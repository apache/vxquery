...
public class ChildAxisIterator extends AbstractLazilyEvaluatedIterator {
    ...
    public ChildAxisIterator(RegisterAllocator rAllocator, RuntimeIterator input, Filter<XDMValue> filter) {
        super(rAllocator);
        this.input = input;
        this.filter = filter;
    ...
    public Object next(CallStackFrame frame) throws SystemException {
        ...
        while (true) {
            ...
            if (node.hasChildren()) {
                sequence.set(regs, node.getChildren());
            }
        }
    }
}