...
public class FnNilledIterator extends AbstractEagerlyEvaluatedFunctionIterator {
    ...
    public Object evaluateEagerly(CallStackFrame frame) throws SystemException {
        XDMNode node = (XDMNode) arguments[0].evaluateEagerly(frame);
        if (node == null || node.getDMOKind() != DMOKind.ELEMENT_NODE) {
            return null;
        }
        return node.getIsNilled() ? BooleanValue.TRUE : BooleanValue.FALSE;
    }
}