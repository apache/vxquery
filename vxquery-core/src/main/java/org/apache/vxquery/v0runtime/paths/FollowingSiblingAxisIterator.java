...
public class FollowingSiblingAxisIterator extends AbstractLazilyEvaluatedIterator {
    ...
    public FollowingSiblingAxisIterator(RegisterAllocator rAllocator, RuntimeIterator input, Filter<XDMValue> filter) {
        super(rAllocator);
        this.input = input;
        this.filter = filter;
    ...
    public Object next(CallStackFrame frame) throws SystemException {
        ...
        while (true) {
            ...
            sequence.set(regs, node.getFollowingSiblings());
        }
    }
}