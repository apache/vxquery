...
public class CastIterator extends AbstractEagerlyEvaluatedIterator {
    ...
    public CastIterator(RegisterAllocator rAllocator, RuntimeIterator input, CastProcessor castProcessor, StaticContext ctx) {
        super(rAllocator);
        this.input = input;
    ...
    public Object evaluateEagerly(CallStackFrame frame) throws SystemException {
        return castProcessor.cast(frame.getRuntimeControlBlock().getAtomicValueFactory(), (XDMValue) input
                .evaluateEagerly(frame), DefaultSystemExceptionFactory.INSTANCE, ctx);
    }
}