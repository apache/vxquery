...
public class FnPrefixFromQNameIterator extends AbstractEagerlyEvaluatedFunctionIterator {
    ...
    public Object evaluateEagerly(CallStackFrame frame) throws SystemException {
        QNameValue v = (QNameValue) arguments[0].evaluateEagerly(frame);
        if (v == null || !NameCache.hasPrefix(v.getCode())) {
            return null;
        }
        return frame.getRuntimeControlBlock().getAtomicValueFactory().createNCName(
                v.getNameCache().getPrefix(v.getCode()));
    }
}