...
public class JSONParser implements IParser {
    ...
    protected List<Object> valueSequence;
    ...
    public JSONParser(List<IPointable> valuePointables) {
        ...
        for (int i = 0; i < valuePointables.size(); i++) {
            ...
            if (((TaggedValuePointable) valuePointables.get(i)).getTag() == ValueTag.XS_INTEGER_TAG) {
                ...
                valueSequence.add(new Long(LongPointable.getLong(valuePointables.get(i).getByteArray(), start)));
                ...
                valueSequence
                        .add(new Boolean(BooleanPointable.getBoolean(valuePointables.get(i).getByteArray(), start)));
            } else {
                //access an object
    ...
    public void atomicValues(int tag, JsonParser parser, DataOutput out, StringValueBuilder svb, int levelArray,
        ...
        if (!itemStack.isEmpty()) {
            if (itemStack.get(itemStack.size() - 1) == itemType.ARRAY) {
                ...
                if (!valuePointables.isEmpty() && this.matched && levelArray == this.arrayMatchLevel) {
                    this.literal = true;
                    this.matched = false;
                ...
                if (!valuePointables.isEmpty() && this.matched && levelObject == this.objectMatchLevel) {
                    this.literal = true;
                    this.matched = false;
                    writeElement(abvsStack.get(0));
                }
